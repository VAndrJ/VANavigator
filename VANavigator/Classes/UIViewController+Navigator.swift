...
extension UIViewController {
    ...
    public func findController(controller: UIViewController, withPresented: Bool) -> UIViewController? {
        if self === controller {
            return self
        } else if let navigation = self as? UINavigationController {
            for child in navigation.viewControllers {
                if let target = child.findController(controller: controller, withPresented: withPresented) {
                    return target
                }
            }
            ...
            return presentedViewController.findController(controller: controller, withPresented: withPresented)
        }

        return nil
    }

    public func findController(identity: NavigationIdentity, withPresented: Bool) -> UIViewController? {
        if navigationIdentity?.isEqual(to: identity) == true {
            return self
        } else if let navigation = self as? UINavigationController {
            for controller in navigation.viewControllers {
                if let target = controller.findController(identity: identity, withPresented: withPresented) {
                    return target
                }
            }
        } else if let tab = self as? UITabBarController {
            ...
            return presentedViewController.findController(identity: identity, withPresented: withPresented)
        }
    ...
    public func findController(destination: NavigationDestination, withPresented: Bool = false) -> UIViewController? {
        switch destination {
        case let .identity(identity):
            return findController(identity: identity, withPresented: withPresented)
        case let .controller(controller):
            return findController(controller: controller, withPresented: withPresented)
        }
    }
}
